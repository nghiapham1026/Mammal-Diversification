...
amphibian_data_path = '../data/processed/taxon/interpolated/Amphibian_Diversity_interpolated.csv'
...
amphibian_df = pd.read_csv(amphibian_data_path)
animal_df = pd.read_csv(animal_data_path)

# Preprocessing
climate_df = climate_df[['Time (Myr BP)', 'Ts']].dropna()  # Ensure there are no NaN values in 'Ts'
amphibian_df = amphibian_df[['max_ma', 'sampled_in_bin']].dropna()  # Ensure no NaN values in 'sampled_in_bin' for amphibians
animal_df = animal_df[['max_ma', 'sampled_in_bin']].dropna()  # Same for total animal data

# Merge amphibian and total animal data on 'max_ma' to calculate the diversity ratio
diversity_ratio_df = pd.merge(amphibian_df, animal_df, on='max_ma', suffixes=('_amphibian', '_total'))
diversity_ratio_df['diversity_ratio'] = diversity_ratio_df['sampled_in_bin_amphibian'] / diversity_ratio_df['sampled_in_bin_total']
...
plt.title('Correlation between Surface Temperature and Amphibian-to-Total Animal Diversity Ratio')
plt.xlabel('Surface Temperature (Ts)')
plt.ylabel('Amphibian-to-Total Animal Diversity Ratio')
