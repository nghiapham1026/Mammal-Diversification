...
insect_data_path = '../data/processed/taxon/interpolated/Insect_Diversity_interpolated.csv'
...
insect_df = pd.read_csv(insect_data_path)
animal_df = pd.read_csv(animal_data_path)

# Preprocessing
climate_df = climate_df[['Time (Myr BP)', 'Ts']].dropna()  # Ensure there are no NaN values in 'Ts'
insect_df = insect_df[['max_ma', 'sampled_in_bin']].dropna()  # Ensure no NaN values in 'sampled_in_bin' for insects
animal_df = animal_df[['max_ma', 'sampled_in_bin']].dropna()  # Same for total animal data

# Merge insect and total animal data on 'max_ma' to calculate the diversity ratio
diversity_ratio_df = pd.merge(insect_df, animal_df, on='max_ma', suffixes=('_insect', '_total'))
diversity_ratio_df['diversity_ratio'] = diversity_ratio_df['sampled_in_bin_insect'] / diversity_ratio_df['sampled_in_bin_total']
...
plt.title('Correlation between Surface Temperature and Insect-to-Total Animal Diversity Ratio')
plt.xlabel('Surface Temperature (Ts)')
plt.ylabel('Insect-to-Total Animal Diversity Ratio')
